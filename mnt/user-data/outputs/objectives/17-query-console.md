...
export class QueryConsoleService {
  ...
  ): Promise<Result<QueryResult, AppError>>;
  ...
  async exportResult(
    ctx: RequestContext,
    input: ExportInput,
  ): Promise<Result<{ exportId: string; downloadUrl: string }, AppError>>;
}
```
...
*This document is the contract. Every checkbox in Section 10 must be true before moving on to Objective 18.*
