...
// Fully typed:
const { data } = await platform.data('users').select('id', 'email').where({ active: { _eq: true } });
...
Users should write:
```typescript
const { data, error, count } = await platform.data('posts')
...
const { data, error } = await platform.data('posts').where({ id: { _eq: '...' } }).update({ title: 'New title' });
const { error } = await platform.data('posts').where({ id: { _eq: '...' } }).delete();
```
...
*This document is the contract. Every checkbox in Section 10 must be true before declaring the Data Management Module complete and ready for customers.*
