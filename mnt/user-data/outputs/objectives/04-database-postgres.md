...
- The actual platform schema (workspaces, users, projects, artifacts) — this objective implements the *mechanics* of persistence, not the platform's own schema. The schema lands when each module needs its tables (Auth/User Directory in the Data Management Module, etc.)
...
): Result<TranslatedFilter, FilterError> {
