...
2. **The Identity Provider system**: pluggable adapters that *verify* who someone is. The default is the platform's own built-in auth (the Supabase-clone auth feature). Alternative adapters cover Entra ID, generic OIDC, generic SAML — for enterprise customers who already have an IdP.
...
**Key invariant:** every authenticated user in the platform has a User Directory record, regardless of which IdP verified them. The identity provider is *who you are externally*; the User Directory record is *who you are inside the platform*.
