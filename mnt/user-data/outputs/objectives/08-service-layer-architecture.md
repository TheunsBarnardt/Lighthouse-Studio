...
export function composeServices(adapters: PlatformContainer): ServiceContainer {
  ...
  const workspaceService = new WorkspaceService(
    ...
    adapters.persistence.repositories.workspaces,
    adapters.persistence.repositories.members,
    adapters.audit,
    baseLogger.child({ service: 'WorkspaceService' }),
  );

  const memberService = new MemberService(
    adapters.authorization,
    adapters.persistence.repositories.members,
    adapters.persistence.repositories.invitations,
    adapters.email,
    adapters.audit,
    baseLogger.child({ service: 'MemberService' }),
  );
...
export function observable<T extends (...args: any[]) => Promise<any>>(
  service: string,
  method: string,
  fn: T,
): T {
...
describe('WorkspaceService.create', () => {
  ...
  describe('with in-memory adapters', () => {
    ...
    it('emits an audit event on success', async () => {
      ...
      expect(adapters.audit.events).toContainEqual(expect.objectContaining({
        eventType: 'workspace.created',
      }));
    });
  });
...
*This document is the contract. Every checkbox in Section 20 must be true before moving on.*
