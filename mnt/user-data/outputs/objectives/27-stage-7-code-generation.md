...
export class CodeGenerationService {
  ...
  ): Promise<Result<Artifact<FunctionInventory>, AppError>>;

  /** Generate full server code (all functions in inventory). */
  async generateAll(
    ctx: RequestContext,
    inventoryArtifactId: string,
  ): Promise<Result<Artifact<ServerCodeProject>, AppError>>;
  ...
  ): Promise<Result<Artifact<ServerFunction>, AppError>>;
...
*This document is the contract. Every checkbox in Section 10 must be true before moving on to Objective 28.*
