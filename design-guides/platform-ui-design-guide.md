...
_The design language for the platform's own admin interface. NOT the design language for apps the platform generates — that's the Generated Apps Design Guide. This document covers what customers see when they install the platform, configure it, and operate it._
...
_This guide is the visual contract for the platform UI. Implementation lives in `packages/ui-tokens/` and `packages/ui-components/`. When this guide and a specific objective's UI section conflict, this guide wins for visual decisions; the objective wins for functional decisions._

_Status: v1, written before any UI is built. Will iterate as the first surfaces ship and reveal what's missing._
