...
export function safeAsync<T, E = Error>(promise: Promise<T>, errorMapper?: (e: unknown) => E): ResultAsync<T, E> {
  /* ... */
}
```
...
```javascript
export default {
  '*.{ts,tsx,mts,cts,mjs,cjs,js,jsx}': ['eslint --fix --max-warnings=0', 'prettier --write'],
...
```json
{
  ...
  "extends": ["config:recommended", ":semanticCommits", ":dependencyDashboard", ":maintainLockFilesWeekly"],
