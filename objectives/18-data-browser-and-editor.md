...
export class DataBrowserService {
  ...
  async resolveForeignKeys(ctx: RequestContext, schemaId: string, requests: FkLookupRequest[]): Promise<Result<FkLookupResult[], AppError>>;
}
```
...
_This document is the contract. Every checkbox in Section 10 must be true before moving on to Objective 19._
