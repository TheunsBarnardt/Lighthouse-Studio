...
const PG_KEYWORDS = [
  ...
  'WITH', 'UNION', 'INTERSECT', 'EXCEPT', 'ALL', 'INSERT', 'UPDATE', 'DELETE',
  'INTO', 'VALUES', 'SET', 'RETURNING', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END',
  'IS', 'NULL', 'TRUE', 'FALSE', 'CAST', 'COALESCE', 'NULLIF', 'ASC', 'DESC',
];

const PG_FUNCTIONS = [
  'now()', 'current_timestamp', 'current_date', 'current_time',
  'count(*)', 'count(1)', 'sum()', 'avg()', 'min()', 'max()',
...
const MSSQL_KEYWORDS = [
  'SELECT', 'TOP', 'FROM', 'WHERE', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER',
  ...
  'CAST', 'CONVERT', 'COALESCE', 'ASC', 'DESC', 'NOLOCK', 'GO',
];
...
const MSSQL_FUNCTIONS = [
  ...
  'LEN()', 'LOWER()', 'UPPER()', 'LTRIM()', 'RTRIM()', 'TRIM()',
...
const MONGO_STAGE_NAMES = [
  ...
  '$lookup', '$unwind', '$addFields', '$replaceRoot', '$replaceWith',
  '$count', '$facet', '$bucket', '$bucketAuto', '$sortByCount',
  '$geoNear', '$graphLookup', '$redact', '$sample',
];
...
const MONGO_OPERATORS = [
  ...
  '$concat', '$substr', '$toLower', '$toUpper', '$dateToString',
...
export class QueryConsoleAutocomplete {
  ...
  async suggest(
    ...
    try {
      const schemaResult = await this.schemaService.getById(ctx, schemaId);
      if (schemaResult.isOk() && schemaResult.value) {
          ...
          if (dotPattern.test(textBeforeCursor)) {
            for (const col of table.columns ?? []) {
              items.push({
                label: col.name,
                kind: 'column',
                detail: String(col.type?.kind ?? 'unknown'),
                insertText: col.name,
