...
import type { BriefDraft, BriefFieldStatus, IntentBrief } from '@platform/core';

import { FileText, Edit3, Send, CheckCircle } from 'lucide-react';
import { useState } from 'react';

import { BriefFieldCard } from '../components/BriefFieldCard';
import { MarkdownBriefView } from '../views/MarkdownBriefView';
import { StructuredBriefView } from '../views/StructuredBriefView';

type BriefViewTab = 'structured' | 'markdown';
...
interface BriefPreviewPanelProps {
  ...
  onBriefUpdate?: (updates: Partial<IntentBrief>) => void;
}
...
export function BriefPreviewPanel({
  ...
  const [activeTab, setActiveTab] = useState<BriefViewTab>('structured');
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900/50">
      ...
      {/* View tab switcher — only shown when a brief has been generated */}
      {briefId && brief && (
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Generated brief views */}
        {briefId && brief ? (
          activeTab === 'markdown' ? (
            <MarkdownBriefView
              brief={brief}
              onUpdate={onBriefUpdate ?? (() => undefined)}
            />
          ) : (
            <StructuredBriefView
              brief={brief}
          ...
          /* Draft preview during conversation */
