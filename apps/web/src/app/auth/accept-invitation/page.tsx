...
export default function AcceptInvitationPage() {
  ...
  const [success, setSuccess] = useState<{ workspaceSlug: string; workspaceName: string } | null>(null);
  ...
  useEffect(() => {
    ...
    void authApi.validateInvitation(token)
      .then((inv) => {
        setInvitation(inv);
        return inv;
      })
  ...
  async function handleAccept() {
    ...
    try {
      ...
      setSuccess({ workspaceSlug: result.workspaceSlug, workspaceName: invitation?.workspaceName ?? '' });
    } catch (err) {
  ...
  if (success) {
    return (
      <Card>
        <CardHeader><CardTitle>{t('success', { workspaceName: success.workspaceName })}</CardTitle></CardHeader>
          ...
          <>
            ...
            {!user && (
              <p className="text-sm text-muted-foreground">
                ...
                <Link href={`/auth/sign-in?returnTo=/auth/accept-invitation?token=${token}`} className="text-primary hover:underline">Sign in</Link>
              </p>
            )}
          </>
        )}
