# ADR-0245: Generation-Time SCA Gate
...
Stage 7 (Objective 27) generates server code, including `package.json` files. Today, vulnerable dependencies in generated code are caught only at deploy time by the gate in ADR-0242. That is too late: by the time the user sees the scan result, they have already approved the generated code in the change-review UI. Catching it earlier closes the feedback loop and avoids wasted review cycles.
...
The deploy-time gate (ADR-0242) remains in place — generation-time and deploy-time gates are complementary, not redundant. Generation-time catches issues during AI iteration; deploy-time catches issues introduced by upstream advisories that arrived between generation and deploy.
...
- The same scanner (Grype, ADR-0242) is invoked. No new tool to maintain.
...
- ADR-0242 (deploy-time gate; same scanner)
- ADR-0241 (advisory source)
